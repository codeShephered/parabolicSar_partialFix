...
6-Layer pipeline (per 30-second tick):
  L1 : Fetch Zerodha index LTP
  L2 : Update 5-min candle builder
  L3 : Update Parabolic SAR (on candle close)
  L4 : Exit checks — SAR SL + premium SL (every tick)
...
def _process(instrument: str) -> None:
    # ── L1: index LTP ─────────────────────────────────────────────────────────
    quote = _feed_ok() and zerodha_feed.get_quote(instrument) or {}
    if not quote:
        return
    price = float(quote["last_price"])
    state["market"][instrument] = quote
    ...
    # ── L4 (tick): SAR SL + premium backstop ──────────────────────────────────
    ...
    # SAR must agree with pattern (combined strategy — constraint 4)
...
def _poll() -> None:
    for inst in ("NIFTY", "BANKNIFTY"):
        try:
            ...
            logger.error(f"Error {inst}: {exc}", exc_info=True)
    ...
    state["connected"] = _feed_ok()


def _scheduler() -> None:
    warn_ct = 0
    logger.info(
        f"Scheduler started  mode={state['mode'].upper()}  "
        f"SAR start={config.SAR_START} inc={config.SAR_INCREMENT} max={config.SAR_MAX}"
    )
    while not _stop_evt.is_set():
        if state["running"]:
            if not _feed_ok():
                _reconnect()
                warn_ct += 1
                if warn_ct <= 3 or warn_ct % 20 == 0:
                    logger.warning(f"Zerodha unreachable (attempt {warn_ct})")
            else:
                warn_ct = 0
                _poll()
        _stop_evt.wait(config.POLL_INTERVAL_SECONDS)
...
def _watchdog() -> None:
    ...
    while not _stop_evt.is_set():
        ...
        last = _t.monotonic()
        if elapsed > 90:
            logger.info(f"Woke from sleep (~{int(elapsed-60)}s) — reconnecting")
            _reconnect()
...
def _init() -> None:
    ...
    #         logger.warning("Zerodha auto-connect failed — use dashboard to login")
    # else:
    ...
    if config.ZERODHA_API_KEY and config.ZERODHA_ACCESS_TOKEN:
        ...
        if token_len < 10:
            # Blank or placeholder value — skip auto-connect silently
    ...
    threading.Thread(target=_scheduler, daemon=False, name="Scheduler").start()
